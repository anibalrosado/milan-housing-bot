"""
Step 5 test: Uniplaces scraper against the live site.

Run from project root:
    python tests/test_step5_uniplaces.py

Note: Uniplaces /f/entire-house shows fully furnished whole apartments only.
Inventory varies — 0 results for a given search type is acceptable if the
platform has no matching listings today.
"""
...
def main():
    ...
    print("Running Uniplaces scraper (this may take 60–90 seconds)…\n")

    scraper = UniplacesScraper(config, eur_group, eur_couple)
    ...
    if not listings:
        ...
        print("   Uniplaces may have limited inventory matching the bedroom/budget filters.")
    ...
    for i, l in enumerate(listings):
        ...
        elif "uniplaces.com" not in l.url:
            errors.append(f"Listing {i+1}: URL not from uniplaces.com")
        if not l.title:
    ...
    for l in listings[:8]:
        ...
        avail_str = l.available_from or "N/A"
    ...
    if errors:
        ...
        print(f"✅  Uniplaces scraper working — {len(listings)} listing(s) validated.")
