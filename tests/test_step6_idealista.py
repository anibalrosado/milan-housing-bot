"""
Step 6 test: Idealista scraper against the live site.

Run from project root:
    python tests/test_step6_idealista.py

Note: Idealista uses DataDome bot-detection. If blocked, the scraper returns
0 listings with a warning rather than crashing. Check the log output to see
whether results are from actual scraping or a graceful block.

Room count note: Idealista uses Italian "locali" (rooms). We subtract 1 to
convert to bedrooms (bilocale=2 rooms → 1 bedroom). The test accepts 0
Group-of-5 results since 5-bedroom apartments are rare.
"""
...
def main():
    ...
    print("Running Idealista scraper (this may take 2–3 minutes)…\n")

    scraper = IdealistaScraper(config, eur_group, eur_couple)
    ...
    if not listings:
        ...
        print("   1. DataDome bot-detection blocked the scraper (check WARNING in logs)")
        print("   2. No apartments match the bedroom/budget filters today")
        print("   The scraper did not crash — that is ✓.")
        return
    ...
    for i, l in enumerate(listings):
        ...
        elif "idealista.it" not in l.url:
            errors.append(f"Listing {i+1}: URL not from idealista.it")
        if not l.title:
    ...
    if errors:
        ...
        print(f"✅  Idealista scraper working — {len(listings)} listing(s) validated.")
